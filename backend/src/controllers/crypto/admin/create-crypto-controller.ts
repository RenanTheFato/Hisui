  ...
  async handle(req: FastifyRequest, rep: FastifyReply) {
    ...
    const cryptoValidate = z.object({
      ...
      blockchain: z.string({ error: "The value has entered isn't an string." })
        .min(2, { error: "The crypto sector doesn't meet the minimum number of characters (2)." })
        .max(256, { error: "The crypto sector exceeds the maximum number of characters (256)." }),
